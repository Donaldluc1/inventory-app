...
class Manage
{
	...
	private $con;
		...
		public function manageRecordWithPagination($table,$pno){
			...
			if ($table == "categories") {
				$sql = "SELECT p.category_name as category, c.category_name as parent, p.status, p.cid as cid FROM categories p LEFT JOIN categories c ON p.parent_cat=c.cid ".$a["limit"];
			}else if ($table == "products") {
				$sql = "SELECT `pid`,p.`product_name`,c.`category_name`,b.`brand_name`,p.`product_price`,p.`product_stock`,p.`added_date`,p.`p_status` FROM `products` p, `brands` b, `categories` c WHERE p.`bid` = b.`bid` AND p.`cid` = c.`cid`".$a["limit"];
			} else if ($table == "user") {
				echo $sql = "SELECT `id`,`username`, `email`, `password`, `usertype` FROM `user` WHERE user.id = ".$_SESSION["userid"]." ".$a["limit"];
			}else{
				$sql = "SELECT `bid`, `brand_name`, `status` FROM `$table`".$a["limit"];
			}
...
//$obj = new Manage();
//echo "<pre>";
//print_r($obj->manageRecordWithPagination("user",1));
