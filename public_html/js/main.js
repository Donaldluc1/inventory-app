$(document).ready(function(){
	...
	$("#register_form").on("submit",function(){
		...
		if (type.val() == "") {
			...
			$("#t_error").html("<span class='text-danger'>Please Choose a Type</span>");
			status = false;
		}else{
			...
			$.ajax({
			   ...
			   success : function(data){
			   		if (data == "EMAIL_ALREADY_EXISTS") {
			   			...
			   			$(".overlay").hide();
			   			window.location.href = encodeURI(DOMAIN+"/index.php?msg=You are registred Now you can login");
			   		}
			   }
			})
		}else{
	...
	$("#form_login").on("submit",function(){	
		...
		if (status) {
			...
			$.ajax({
			   ...
			   success : function(data){
			   		if (data == "NOT_REGISRED") {
			   			$(".overlay").hide();
			   			email.addClass("border-danger");
						$("#e_error").html("<span class='text-danger'>It seems like you are not registred</span>");
